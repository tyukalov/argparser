...
argparse(const ARGPARSE_ARG args)
{
  ...
  varval - pointers to option parameter;
  ...
  for(count=1; count < args.argc; count++) {
...
#ifdef ALT_SEPARATOR
	     || (varval=strchr(popt, (int)ALT_SEPARATOR))
#endif
	 ){
	strncpy(buf, popt, (varval - popt));
	buf[varval-popt] = 0;			/* trailing zero */
	varval = ++varval;				/*cut out the symbol of the separator  */
	if (!strlen(varval))
	  varval = NULL;
      } else {
	strcpy(buf, popt);
	...
	if(args.opts[gonumber].flag == AP_REQUIRED || args.opts[gonumber].flag == AP_OPTIONAL){
	  if ((!varval) && (args.separator == AP_SPACE))
	  ...
	  varopt->value = varval;
	  if(args.separator == AP_SPACE && (args.opts[gonumber].flag == AP_REQUIRED || args.opts[gonumber].flag == AP_OPTIONAL))
	    valflag = FALSE;
	}
	varopt->option = args.opts[gonumber].options;
      } else{
	numcopts = strlen(buf) - 1;
	for(; numcopts>=0; numcopts--){ /*When passing an optional parameter, the option must be seperate! */
